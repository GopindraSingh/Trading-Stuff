...
def scan_momentum_sellers(mode="1", target_date_str=None):
    if mode == "1":
        ...
        download_kwargs = {"period": "2d"}
    else:
        current_date = target_date_str
        target_dt = datetime.strptime(current_date, "%Y-%m-%d")
        # Yahoo Finance 'end' date parameter is exclusive, so add 1 day to catch target date's intraday bars
    ...
    for ticker in tickers:
        try:
            ...
            # Filter rows matching the scan date and the 09:15 market open time
...
if __name__ == "__main__":
    ...
    if not all_candidates.empty:
        ...
        print("No stocks matched the general breakdown criteria on this date.")
