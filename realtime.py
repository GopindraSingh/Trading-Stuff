...
def scan_momentum_sellers(mode="1", target_date_str=None):
    if mode == "1":
        ...
        download_kwargs = {"period": "5d"}  # 5 days needed to calculate average opening volume baseline
    else:
        current_date = target_date_str
        target_dt = datetime.strptime(current_date, "%Y-%m-%d")
        start_dt = target_dt - timedelta(days=7)  # Pull a week prior for volume history baseline
    ...
    for ticker in tickers:
        try:
            ...
            # Isolate all 09:15 opening candles to compute average baseline volume
            ...
            # Calculate average opening volume of past sessions (excluding current target day)
            target_dt_obj = pd.to_datetime(current_date).date()
            past_openings = all_openings[all_openings.index.date < target_dt_obj]
            avg_opening_vol = past_openings["Volume"].mean() if not past_openings.empty else all_openings["Volume"].mean()

            # Target date 09:15 candle extraction
            today_mask = (ticker_df.index.date == target_dt_obj) & (ticker_df.index.time == market_open_time)
            ...
            if not opening_candle.empty:
                ...
                # --- Backtesting Outcome Engine ---
                # Check subsequent candles on the same day after 09:15
                subsequent_candles = ticker_df[(ticker_df.index.date == target_dt_obj) & (ticker_df.index.time > market_open_time)]
                
                target_price = close_p * 0.99  # 1% Target down
                stop_loss = high_p             # High of 09:15 candle
                ...
                # Tier 1: General Losers (Requires at least 1.2x average opening volume spike)
                if (
                    candle_drop_pct < -0.5
                    and turnover_cr >= 4.0
                ...
                # Tier 2: Shortlisted Most Falling (Requires 2.0x volume spike)
                if (
                    candle_drop_pct < -1.5
    ...
    if not df_all.empty:
        df_all = df_all.sort_values(by="Candle_Drop_%", ascending=True).reset_index(drop=True)

    df_short = pd.DataFrame(shortlisted_losers)
    if not df_short.empty:
        df_short = df_short.sort_values(by="Candle_Drop_%", ascending=True).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    all_candidates, shortlisted_candidates = scan_momentum_sellers(choice, target_date)
